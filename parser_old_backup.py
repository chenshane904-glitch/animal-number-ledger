...
class InstructionParser:
    ...
    def _parse_line(self, line: str, line_num: int) -> Instruction:
        ...
        # 移除可能的前缀（如"门:"、"生肖:"等）
        # 查找冒号，如果存在且冒号前只有少量字符，移除前缀
        colon_pos = normalized.find(':')
        if colon_pos != -1 and colon_pos <= 10:  # 前缀不超过10个字符
            # 检查冒号前是否只有文字（不是号码的一部分）
            prefix = normalized[:colon_pos].strip()
            # 如果前缀不包含数字，认为是标签前缀
            if not any(c.isdigit() for c in prefix):
                normalized = normalized[colon_pos + 1:].strip()
